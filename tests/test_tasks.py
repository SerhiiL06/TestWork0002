...
async def test_create_task(
    transport: ASGITransport, auth_headers: dict, correct_task: dict
) -> None:
...
async def test_get_tasks(transport: ASGITransport, auth_headers: dict) -> None:
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        ...
        response = await ac.get(
            url="/tasks", headers=auth_headers, params={"priority": 2}
        )
        assert response.status_code == 200
        assert len(response.json().get("tasks")) == 0

        response = await ac.get(
            url="/tasks", headers=auth_headers, params={"status": "PENDING"}
        )
        assert response.status_code == 200
        assert len(response.json().get("tasks")) == 1
...
async def test_update_task(
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        ...
        response = await ac.put(
            url=f"/tasks/{task_id}", headers=auth_headers, json={"status": "DONE"}
        )
        assert response.status_code == 200
        assert response.json().get("task").get("status") == "DONE"

        response = await ac.put(
            url=f"/tasks/{task_id}", headers=auth_headers, json={"status": "TEST"}
        )
        assert response.status_code == 422

        response = await ac.put(
            url=f"/tasks/{task_id}", headers=auth_headers, json={"priority": "10"}
        )
        assert response.status_code == 400
...
async def test_search_task(
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        ...
        response = await ac.get(
            url="/tasks/search", headers=auth_headers, params={"q": "test"}
        )
        assert response.status_code == 200
        assert len(response.json().get("tasks")) == 1

        response = await ac.get(
            url="/tasks/search", headers=auth_headers, params={"q": "random"}
        )
        assert response.status_code == 200
        assert len(response.json().get("tasks")) == 0
